...
SELLER_KEEP = 0.70
REQUEST_DELAY = 1.5
DAILY_ROBUX_CAP = int(os.getenv("DAILY_ROBUX_CAP", "15000"))  # change via Railway env if you want
# ====================================================
...
def get_item_info(asset_id: int) -> Optional[dict]:
    """More resilient: catalog → resellers → economy name → retry once."""
    name = None
    lowest = None
    second = None
    url = f"https://www.roblox.com/catalog/{asset_id}/"
    ...
    # Retry once if still nothing (rate limit recovery)
    if lowest is None:
        time.sleep(1.2)
        resellers = get_resellers_full(asset_id)
    ...
    if not name:
        ...
        name = (eco or {}).get("name") or f"ID {asset_id}"
...
def attempt_purchase(asset_id: int, locked_max: int) -> Tuple[bool, str, int]:
    """Returns (success, message, price_paid_or_0)."""
    if not ROBLOSECURITY:
    ...
    if price > locked_max:
        return False, (
            f"Price increased. Locked at `{locked_max:,}` but current is `{price:,}`."
        ), 0
    ...
    if spent + price > DAILY_ROBUX_CAP:
        return False, (
            f"Daily Robux cap reached. Spent `{spent:,}` / cap `{DAILY_ROBUX_CAP:,}`. "
            f"This buy is `{price:,}`."
        ), 0
...
async def try_auto_buy(info: dict, max_price: int):
    ...
    success, message, paid = await asyncio.to_thread(attempt_purchase, asset_id, locked)
    ...
    if success:
        embed = discord.Embed(
            ...
            description=(
                f"**{name}**\n{message}\n"
                f"Max `{max_price:,}` | Today spent `{get_spent_today():,}` / `{DAILY_ROBUX_CAP:,}`\n\n"
                f"[Open]({info['url']})"
            ),
            color=0x57F287,
            timestamp=datetime.now(timezone.utc),
...
async def price_monitor():
    ...
    for asset_id, max_p in list(ITEMS.items()):
        if asset_id in PURCHASE_TARGETS:
            ...
            continue  # fast loop handles these
...
async def monitorall(interaction: discord.Interaction):
    ...
    try:
        await interaction.followup.send("🔍 Running full check…")
...
async def targetitem(interaction: discord.Interaction, asset_id: int, max_price: int):
    ...
    try:
        ...
        await interaction.followup.send(
            f"✅ Watching **{asset_id}** max `{max_price:,}` (alert only, no auto-buy)"
        )
    except Exception as e:
        await interaction.followup.send(f"Error: `{e}`", ephemeral=True)
...
async def targetpurchase(interaction: discord.Interaction, asset_id: int, max_price: int):
    ...
    try:
        ...
        save_purchase_targets()
        await interaction.followup.send(
            f"🛒 **AUTO-BUY on** `{asset_id}` ≤ `{max_price:,}` R$\n"
            f"Fast loop every {AUTO_BUY_INTERVAL}s | Cap `{DAILY_ROBUX_CAP:,}`/day"
        )
    except Exception as e:
        await interaction.followup.send(f"Error: `{e}`", ephemeral=True)

@bot.tree.command(name="pause", description="Pause all auto-buy (alerts still work)")
async def pause_cmd(interaction: discord.Interaction):
    global AUTO_BUY_PAUSED
    await interaction.response.defer()
    AUTO_BUY_PAUSED = True
    await interaction.followup.send("⏸️ **Auto-buy PAUSED.** Alerts still run.")
...
async def resume_cmd(interaction: discord.Interaction):
    ...
    await interaction.response.defer()
    AUTO_BUY_PAUSED = False
    await interaction.followup.send("▶️ **Auto-buy RESUMED.**")
...
async def showlist(interaction: discord.Interaction):
    ...
    try:
        ...
        lines = [
            f"**{aid}** → max `{price:,}`"
            + (" 🛒 AUTO" if aid in PURCHASE_TARGETS else "")
...
@bot.tree.command(name="status", description="Bot status, pause state, daily spend")
