"""
Roblox Limited Price Notifier - Final Version
"""
...
def send_startup():
    ...
    if success:
        print("✅ Webhook working — 'monitor started' sent")
    else:
...
def main():
    ...
    # Immediate webhook test
    send_startup()

    # Force check all items at startup
    ...
    for asset_id, (min_p, max_p) in ITEMS.items():
        ...
        if info:
            ...
            print(f"  ✅ {info['name']}: {info['lowest']:,} (want {min_p:,}–{max_p:,}) {status}")
        time.sleep(0.7)
        ...
        for asset_id, (min_p, max_p) in ITEMS.items():
            ...
            if min_p <= lowest <= max_p:
                ...
                if now - last >= ALERT_COOLDOWN:
                    ...
                    print(f"  ⏳ {info['name']} in range (cooldown {remaining}s)")
            else:
