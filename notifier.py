...
- /targetitem  → alert + green Buy button
- /targetpurchase → AUTO-BUY when price ≤ max
- defer() on all commands • asyncio.to_thread for Roblox API
"""
...
def load_purchase_targets() -> Dict[int, int]:
    if PURCHASE_FILE.exists():
        try:
            ...
            print("Failed to load purchase_targets.json:", e)
    return {}
...
def save_purchase_targets():
    try:
        ...
        print("Failed to save purchase_targets.json:", e)
...
async def send_alert(info: dict, max_price: int):
    """Normal alert + green Buy button (for /targetitem items)."""
    lowest = info["lowest"]
...
async def try_auto_buy(info: dict, max_price: int):
    """Auto-buy ONLY for /targetpurchase items. Re-checks price first."""
    asset_id = info["asset_id"]
    name = info["name"]
    locked = info["lowest"]

    success, message = await asyncio.to_thread(attempt_purchase, asset_id, locked)
    ...
    if success:
        ...
        # Still send a normal alert so you can buy manually if you want
        body = build_profit_text(locked, info.get("second"), info.get("rap"), max_price)
        embed = discord.Embed(
            title="❌ AUTO-BUY REFUSED / FAILED",
            description=(
                f"**{name}**\n"
                f"{message}\n\n"
                f"{body}\n\n"
        ...
        view = BuyView(asset_id=asset_id, locked_price=locked, item_name=name)
        await send_to_channel_or_webhook(
    ...
    for asset_id, max_p in list(ITEMS.items()):
        ...
        if lowest <= max_p:
            ...
            if now - last >= ALERT_COOLDOWN:
                # AUTO-BUY only if marked with /targetpurchase
...
async def price_report_task():
    ...
    for asset_id, max_p in list(ITEMS.items()):
        ...
        lines.append(f"**{info['name']}**{tag}: `{lowest:,}`{extra}{profit_note} {status}")
        await asyncio.sleep(REQUEST_DELAY)
...
async def on_ready():
    ...
    await send_to_channel_or_webhook(
        content="**monitor started** ✅ Auto-buy enabled for /targetpurchase items"
    )
...
@bot.tree.command(name="targetitem", description="Watch item – alert + Buy button only (no auto-buy)")
...
async def targetitem(interaction: discord.Interaction, asset_id: int, max_price: int):
    ...
    try:
        ...
        # Remove from auto-buy list if it was there
        if asset_id in PURCHASE_TARGETS:
            del PURCHASE_TARGETS[asset_id]
            save_purchase_targets()
        save_items()
        await interaction.followup.send(
            f"✅ Watching **{asset_id}** with max `{max_price:,}` R$\n"
            f"Will **alert** (with Buy button) when lowest ≤ `{max_price:,}`.\n"
            f"*Not* set for auto-buy."
        )
    except Exception as e:
        print(f"/targetitem error: {e}")
        await interaction.followup.send(f"Error: `{e}`", ephemeral=True)

@bot.tree.command(name="targetpurchase", description="Watch item + AUTO-BUY when price ≤ max")
@app_commands.describe(asset_id="Roblox asset ID", max_price="Max price – bot will auto-buy at or below this")
async def targetpurchase(interaction: discord.Interaction, asset_id: int, max_price: int):
    ...
    try:
        ...
        await interaction.followup.send(
            ...
            f"Bot will **automatically buy** when lowest seller is ≤ `{max_price:,}`."
        )
    except Exception as e:
...
async def showlist(interaction: discord.Interaction):
    ...
    try:
        ...
        for aid, price in ITEMS.items():
            tag = " 🛒 AUTO-BUY" if aid in PURCHASE_TARGETS else ""
