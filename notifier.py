...
Persistent targets • Better price fetching • Green Buy button • Gap profits
"""
...
SELLER_KEEP = 0.70
REQUEST_DELAY = 1.4          # seconds between Roblox API calls (helps rate limits)
# ====================================================
...
def get_economy_name(asset_id: int) -> Optional[str]:
    try:
        ...
        if r.status_code == 200:
            return r.json().get("Name") or r.json().get("name")
    except Exception:
        pass
    return None
...
def get_item_info(asset_id: int) -> Optional[dict]:
    """
    More resilient price fetch:
    1) catalog details
    2) resellers list (best source for live lowest)
    3) economy name fallback
    """
    name = None
    lowest = None
    second = None
    ...
    if lowest is None:
        # Last attempt: sometimes catalog fails but we still want a name
        if not name:
...
async def on_ready():
    ...
    await send_to_channel_or_webhook(
        content="**monitor started** ✅ Bot online (persistent targets + Buy button)"
    )
...
async def remove_item(interaction: discord.Interaction, asset_id: int):
    ...
    if removed:
        save_items()
        await interaction.response.send_message(f"🗑️ Removed **{asset_id}**.")
    else:
