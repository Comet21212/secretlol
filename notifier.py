...
Accurate 30% tax • 1st vs 2nd seller • RAP attempt • 15-min reports
"""
...
PRICE_REPORT_INTERVAL = 15 * 60          # every 15 minutes

# Real 2026 marketplace fee (seller keeps 70%)
SELLER_KEEP = 0.70
# ====================================================
...
last_uptime = 0.0
next_price_report = time.time() + 45          # first report ~45s after start
...
def build_profit_text(lowest: int, second: Optional[int], rap: Optional[int], max_price: int) -> str:
    ...
    lines.append(f"**You receive after 30% tax:** `{net:,}` R$")
...
def send_price_report():
    ...
    embed = {
        ...
        "color": 0x5865F2,
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "footer": {"text": "1st → 2nd seller + possible profit after 30% tax"},
    }
    send_webhook(embeds=[embed])
    print("📊 Report sent\n")
...
def main():
    ...
    print("=" * 55)
    print("🚀 SMARTER NOTIFIER – Version A (15 min reports)")
    print("=" * 55)
