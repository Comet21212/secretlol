"""
Roblox Limited Notifier – Hardened
Dual resellers (legacy + collectible) • smarter suggested sell
...
CHECK_INTERVAL = 50
AUTO_BUY_INTERVAL = 5          # slightly slower = fewer rate limits
ALERT_COOLDOWN = 300
...
MIN_AUTO_PROFIT = 0            # set e.g. 200 to only auto-buy if after-tax profit >= 200
# ====================================================
...
def resolve_ids(asset_id: int) -> dict:
    """product_id + collectible_item_id for purchase routing."""
    out = {
        "name": f"ID {asset_id}",
        "product_id": None,
    ...
    if not out["product_id"] or not out["collectible_item_id"]:
        ...
        if eco:
            ...
            # some payloads use CollectibleItemId
...
def _parse_collectible_resellers(data: list) -> List[dict]:
    ...
    for row in data or []:
        price = row.get("price")
        # field names vary slightly across versions
        cpid = (
        ...
        uaid = row.get("userAssetId")  # may be absent on collectibles
        if price is None:
            continue
        rows.append({
            "price": int(price),
...
def get_resellers_full(asset_id: int, collectible_item_id: Optional[str] = None) -> List[dict]:
    """Try legacy economy resellers, then collectible marketplace-sales resellers."""
    if not ROBLOSECURITY:
        return []
...
def get_item_info(asset_id: int) -> Optional[dict]:
    ...
    return {
        ...
        "url": ids["url"],
        "resellers_sample": resellers[:3] if resellers else [],
    }

def suggest_sell(lowest: int, second: Optional[int], rap: Optional[int], max_price: int) -> Tuple[int, str]:
    """2nd seller → RAP → break-even/your max. Never suggest below after-tax break-even."""
    ...
    # No market signal: don't use weak max*0.92
    suggested = max(be, max_price)
    return suggested, "break-even / your max"
...
def attempt_purchase(asset_id: int, locked_max: int) -> Tuple[bool, str, int]:
    ...
    # Optional profit gate using suggested sell
    rap = get_rap(asset_id)
    ...
    if MIN_AUTO_PROFIT and projected < MIN_AUTO_PROFIT:
        return False, (
            f"Profit gate: projected after-tax `{projected:,}` < min `{MIN_AUTO_PROFIT:,}` "
    ...
    if cid and cpid and ROBLOX_USER_ID:
        ...
        try:
            ...
            try:
                err = r.json()
                msg = err.get("message") or err.get("errorMessage") or str(err)[:200]
            except Exception:
                msg = r.text[:200]
            # fall through to legacy if collectible path rejects
    ...
    # Path B: legacy tradable limited
    ...
    try:
        ...
        try:
            err = r.json()
            msg = err.get("message") or err.get("errors", [{}])[0].get("message") or str(err)[:200]
        except Exception:
            msg = r.text[:200]
        return False, f"Legacy purchase failed (HTTP {r.status_code}): {msg}", 0
    except Exception as e:
        return False, f"Legacy purchase error: {e}", 0
...
async def on_ready():
    ...
    await send_to_channel_or_webhook(
        content=(
            ...
            f"Resellers: legacy + collectible | Auto-buy every `{AUTO_BUY_INTERVAL}s`"
        )
    )
